...
public class HandTrackingLetians : MonoBehaviour
{
    // declare variables //
...
//    public GameObject axis;
    ...
    void Update()
    {
        ...
        */
        ...
        //hand_axis.GetComponent<Renderer>().enabled = false;
        ...
        if (HandJointUtils.TryGetJointPose(TrackedHandJoint.Palm, hand, out pose))
        {
            ...
            //hand_axis.GetComponent<Renderer>().enabled = true;
        ...
        if (teleop & CalibrationScript.calib_completed)
        {
            ...
            if (thumb.GetComponent<Renderer>().enabled)
            {
                ...
                if (pinch_dist < clutch_dist + 0.01 && !cameraisMoving /*Calib.calibrated_pinch*/)
                {
                    first_pinch = !teleop_lock;
                    teleop_lock = true;
                    //ChangeQuadBgColor("clutch");
                }
                else if (pinch_dist < clutch_dist + 0.01 && cameraisMoving)
                {
                    teleop_lock = false;
                    /*
                ...
                else if (pinch_dist > clutch_dist + 0.01 || cameraisMoving/* (Calib.calibrated_pinch + 0.011)*/)    // only stop teleop if pinch dist greater than certain threshold
                {
                    teleop_lock = false;
                    //ChangeQuadBgColor("release");
                }
