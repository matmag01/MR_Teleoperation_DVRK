...
public class InstrumentGripperNew : MonoBehaviour
{
    ...
    private GameObject cylinderPSM2Bis;
    private GameObject cylinderPSM1Bis;
    ...
    // Filtro posizione e direzione per PSM1 e PSM2
    private MotionFilter midpointFilterPSM1 = new MotionFilter();
    private MotionFilter midpointFilterPSM2 = new MotionFilter();
    private MotionFilter directionFilterPSM1 = new MotionFilter();
    private MotionFilter directionFilterPSM2 = new MotionFilter();

    // Per la distanza usiamo un filtro scalare
    private float smoothedDistancePSM1 = 0f;
    private float smoothedDistancePSM2 = 0f;
    public float distanceSmoothingFactor = 0.85f; // stesso principio di smoothingFactor
    ...
    void Update()
    {
        ...
        // End Pixel Position
        zEndPxPSM1 = offsetTipPxPSM1 - dirZPxPSM1 * 5f;
        zEndPxPSM2 = offsetTipPxPSM2 - dirZPxPSM2 * 5f;
        ...
        Debug.Log("Quad Local Scale: " + quad.transform.localScale);
        /*
        ...
        */
        ...
        {
            sphereStart = GameObject.CreatePrimitive(PrimitiveType.Sphere);
            sphereStart.transform.localScale = Vector3.one * 0.008f;
            sphereStart.GetComponent<Renderer>().material.color = Color.green;
            Destroy(sphereStart.GetComponent<Collider>());
            cylinderPSM2 = GameObject.CreatePrimitive(PrimitiveType.Cylinder);
            cylinderPSM2.GetComponent<Renderer>().material.color = Color.gray;
            //cylinderPSM2.GetComponent<Renderer>().material = cylinderMaterial;
            Destroy(cylinderPSM2.GetComponent<Collider>());
        }
        sphereStart.transform.position = worldPosPSM2;
        ...
        cylinderPSM1.transform.localScale = new Vector3(0.005f, distancePSM1 / 2f, 0.005f);
        cylinderPSM2.transform.localScale = new Vector3(0.005f, distancePSM2 / 2f, 0.005f);
        ...
        */
        ...
        Vector3 filteredMidpointPSM1 = midpointFilterPSM1.UpdateEMA(midpointPSM1);
        Vector3 filteredMidpointPSM2 = midpointFilterPSM2.UpdateEMA(midpointPSM2);

        // Filtraggio direzione
        Vector3 filteredDirectionPSM1 = directionFilterPSM1.UpdateEMA(directionPSM1);
        Vector3 filteredDirectionPSM2 = directionFilterPSM2.UpdateEMA(directionPSM2);

        // Filtraggio distanza (EMA per float)
        smoothedDistancePSM1 = distanceSmoothingFactor * distancePSM1 + (1f - distanceSmoothingFactor) * smoothedDistancePSM1;
        smoothedDistancePSM2 = distanceSmoothingFactor * distancePSM2 + (1f - distanceSmoothingFactor) * smoothedDistancePSM2;

        // Aggiornamento cilindri con valori smoothati
        cylinderPSM1.transform.position = filteredMidpointPSM1;
        cylinderPSM2.transform.position = filteredMidpointPSM2;

        cylinderPSM1.transform.up = filteredDirectionPSM1.normalized;
        cylinderPSM2.transform.up = filteredDirectionPSM2.normalized;

        cylinderPSM1.transform.localScale = new Vector3(0.005f, smoothedDistancePSM1 / 2f, 0.005f);
        cylinderPSM2.transform.localScale = new Vector3(0.005f, smoothedDistancePSM2 / 2f, 0.005f);
