...
public class InstrumentGripperRight : MonoBehaviour
{
    ...
    public Material cylinderMaterialRight;
    float baselineCamera;
    ...
    void Update()
    {
        ...
        */
        ...
        if (cylinderPSM2 == null)
        {
            ...
            cylinderPSM2.GetComponent<Renderer>().material = cylinderMaterialRight;
    ...
    float TriangulateStereoPixel(Vector2 pixelL, Vector2 pixelR, float f, float cx, float cy, float baseline)
    {
        float disparity = pixelL.x - pixelR.x;
        if (Mathf.Abs(disparity) < 1e-6f)
            return 0f; 
        
        float Z = f * baseline / disparity;
